...
if [ $SHLVL -gt 2 ]; then
  ...
  TEMPERATURE=1e-0
  RUN_IDENTIFIER="leobianco/${DATASET}_PERL_seed_${SEED}_episodes_${TOTAL_EPISODES}_lora_${LORA_RANK}_lr_${LEARNING_RATE}_klcoeff_${KL_COEFF}_rlook_${RLOO_K}_ppoepochs_${NUM_PPO_EPOCHS}"
fi
...
accelerate launch \
  ...
  --logging_steps 10 \
  --output_dir "./checkpoints/${DATASET}/perl/${RUN_IDENTIFIER}" \
  --overwrite_output_dir True \
  --push_to_hub True \
  --hub_model_id "$RUN_IDENTIFIER" \
  --dataset_repo_id "leobianco/${DATASET}_perl_processed" \
  ...
  --sft_model_path "leobianco/npov_SFT_seed_130401_epochs_20_lr_5e-5_lora_8" \
  --r "$LORA_RANK" \
  --kl_coef "$KL_COEFF" \
  --rloo_k "$RLOO_K" \
