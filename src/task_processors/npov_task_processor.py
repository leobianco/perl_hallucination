...
class NPOVTaskProcessor(BaseTaskProcessor):
  ...
  def _preprocess_data(self, data: DatasetDict) -> DatasetDict:
    ...
    for split in data.keys():
      ...
      if "has hallucination" in split_data.column_names:
        split_data = split_data.rename_column("has hallucination", "class_hall")
  ...
  def _make_structured_hallucinations_data(
    ...
    This method modifies the input data dictionary by:
      ...
      - Filtering the training set to include only synthetic hallucinations
      (removing organic hallucinations).
      - Filtering the validation set to exclude synthetic hallucinations
      (keeping only organic hallucinations).
      - Assigning the filtered training set to the "train" split and the
      filtered validation set to the "test" split
        in the returned DatasetDict.
    Args:
        data (dict): A dictionary containing dataset splits, typically with
          "train" and "validation" keys, where each value is a dataset
          supporting the `.filter()` method.

    Returns:
        DatasetDict: A HuggingFace DatasetDict with "train" and "test" splits
  ...
  def _make_perl_data(
    ...
    Returns:
        datasets.DatasetDict: DatasetDict with 'train' and 'test' splits for
        PERL.
    """
  ...
  def _make_evaluation_data(self, data: DatasetDict) -> DatasetDict:
    ...
    capped_final_test_set = augmented_dataset.shuffle(seed=args.seed).select(
        range(10000)
    )
  ...
  def _change_hallucination_labels(entry: dict) -> dict:
    ...
    Args:
        entry (dict): Entry with possible label variants for hallucination
          fields.

    Returns:
        dict: Entry with standardized 'class_hall' and 'has synthetic
        hallucination' fields ('Yes'/'No').
    """
  ...
  def _change_omission_labels(entry: dict) -> dict:
    ...
    Returns:
        dict: Entry with standardized 'class_omit' and 'has synthetic coverage
        issue' fields ('Yes'/'No').
    """
  ...
  def _hallucination_labels_to_numerical(entry: dict) -> dict:
    ...
    Args:
        entry (dict): A dictionary containing the key 'class_hall' with values
          'Yes' or 'No'.

    Returns:
        dict: The entry with an added 'class_hall_num' key (0 for 'Yes', 1 for
        'No').
    """
  ...
  def _omission_labels_to_numerical(entry: dict) -> dict:
    ...
    Args:
        entry (dict): A dictionary containing the key 'class_omit' with values
          'Yes' or 'No'.

    Returns:
        dict: The entry with an added 'class_omit_num' key (0 for 'Yes', 1 for
        'No').
    """
  ...
  def get_evaluator_prompt(
    ...
    The returned function has signature func(entry, fewshot_examples=None,
    ...
    def npov_evaluator_prompt(
      ...
      preamble = (
          "<start_of_turn>user\nBelow are examples where an expert linguist"
          " identifies when the neutral natural language rewritings of"
          " arguments used to answer a user query contains additional arguments"
          " not present in the original list.<end_of_turn>\n"
      )

      template = (
          "<start_of_turn>user\nUser query: {user_query}\n{perspective_1_name}"
          " arguments provided: {perspective_1}\n{perspective_2_name} arguments"
          " provided: {perspective_2}\nNeutral point-of-view answer to user"
          " query, rewriting provided arguments in natural"
          " language:{npov_response}\nExpert linguist review: the rewriting of"
          " the provided arguments contains additional arguments not present in"
          " the original list"
          " (Yes/No):<end_of_turn>\n<start_of_turn>model\n{ans}"
      )

      response = (
          entry["npov_response"] if use_true_label else entry["completion"]
      )
  ...
  def _writer_prompt(
    ...
    Args:
        entry (dict): Entry with user query, perspectives, and NPOV response.
        SFT (bool, optional): If True, include the NPOV response in the prompt.
          Defaults to False.
        fewshot_examples (datasets.Dataset, optional): Few-shot examples to
  ...
  def get_formatting_prompts_and_response_template(
    ...
    def formatting_prompts_func(entry: dict) -> list[str]:
      ...
      template_fewshot = (
          "User query: {user_query}\n{perspective_1_name} arguments provided:"
          " {perspective_1}\n{perspective_2_name} arguments provided:"
          " {perspective_2}\nExample neutral point-of-view answer to user"
          " query, rewriting provided arguments in natural"
          " language:\n{npov_response}{eos_token}"
      )
  ...
  def _rm_synthetic_hall_llm(
    ...
    preamble = (
        "SYSTEM:\nYou are a writer tasked with introducing subtle, realistic"
        " hallucinations into neutral answers. The original answer is a"
        " balanced and neutral synthesis of two opposing perspectives,"
        " presenting only the arguments provided in the context. Your goal is"
        " to modify this answer by adding some new piece of information or"
        " argument that is NOT present in the original answer or context. This"
        " addition should be small, plausible, and smoothly integrated,"
        " resembling the kind of factual error that language models often make"
        " when generating text. It can appear as added information on the"
        " existing arguments in the context, or as a new argument on its own.\n"
    )
    ...
    prompt += (
        f"\nTASK:\nQuestion:\n{entry['user_query']}\n\nContext:\nList of"
        f" {entry['perspective_1_name']}"
        f" arguments:\n{entry['perspective_1']}\n\nList of"
        f" {entry['perspective_2_name']}"
        f" arguments:\n{entry['perspective_2']}\n\nNeutral Point-of-View"
        " Answer, without"
        f" hallucinations:\n{entry['npov_response']}\n\nINSTRUCTIONS:\nCreate a"
        " version of the Neutral Point-of-View answer containing"
        " hallucinations by either introducing subtle, hard-to-detect"
        " inaccuracies or misrepresentations to the arguments that were"
        " present in the context, OR by adding one new argument not present in"
        " the context.\nThe hallucinations must respect the following:\n1."
        " Keep most (90%+) of the content faithful to the original arguments."
        " Adding too much text makes hallucinations easy to detect.\n2. Be"
        " consistent with the overall argument flow and keep the tone of the"
        " original answer.\n3. Be presented as factual.\n\nOUTPUT:\nA single,"
        ' self-contained "Modified Answer" containing the subtle'
        " hallucinations (new information not in the context).\n\nModified"
        " Answer with Synthetic Hallucination:\n"
    )
  ...
  def _data_augmentation(data: Dataset) -> dict:
    ...
    Returns:
        dict: Dictionary with augmented data fields for new argument
        combinations.
    """
